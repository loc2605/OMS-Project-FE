    ...
    try {
      ...
      if (response.success) {
        ...
        if (message.toLowerCase().includes('lock')) {
          alert('Tài khoản của bạn đã bị khóa do đăng nhập sai nhiều lần. Vui lòng liên hệ hỗ trợ.');
        } else {
          alert(message);
        }
      }
      ...
      if (message.toLowerCase().includes('lock')) {
        alert('Tài khoản bị khóa. Vui lòng thử lại sau hoặc liên hệ hỗ trợ.');
      } else {
        alert(message);
      }
    }
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            className="form-input block w-full rounded-lg border border-[#e6dfdb] dark:border-stone-700 focus:border-primary focus:ring-primary/20 bg-white dark:bg-stone-800 px-4 py-3.5 text-[#181411] dark:text-white placeholder:text-[#8a7260] transition-all"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            <input
              className="form-input block w-full rounded-lg border border-[#e6dfdb] dark:border-stone-700 focus:border-primary focus:ring-primary/20 bg-white dark:bg-stone-800 px-4 py-3.5 text-[#181411] dark:text-white placeholder:text-[#8a7260] transition-all pr-12"
